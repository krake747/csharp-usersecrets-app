...
Console.WriteLine("Bye UserSecrets App");
...
internal record Password(string Value)
{
    internal (string Hash, byte[] Salt) Hash(HashAlgorithmName hashAlgorithm, int keySize, int iterations)
    {
        ...
        var hash = Rfc2898DeriveBytes.Pbkdf2(Value, salt, iterations, hashAlgorithm, keySize);
    ...
    internal bool Verify(string hash, byte[] salt, HashAlgorithmName hashAlgorithm, int keySize,
        int iterations)
    {
        var hashToCompare = Rfc2898DeriveBytes.Pbkdf2(hash, salt, iterations, hashAlgorithm, keySize);
